...
- Para os dados numéricos que faltavam de informações do anúncio do produto (tamanho do título, quantidade de fotos e tamanho da descrição), usou-se a média. Esses dados entraram na Árvore 
...
py$df_ordem_compra %>% 
    ...
    geom_boxplot() + 
   labs(y = 'Avaliação', x = 'Tamanho do comentário', fill = 'Ter colocado !')
```
...
- Usando todas variáveis, não alcançou explicabilidade mínima na variabilidade de pelo menos uma componente optei por tratar da PCA com as variáveis mais correlacionadas. Ganho:
...
plt.axhline(y=Valor_Esperado_Minimo, color='r', linestyle='--', label='y=1')  # Linha vermelha em y=1, linha pontilhada
plt.show()
```
...
df_agrupamento_regiao_pivot = df_agrupamento_regiao.pivot_table(
